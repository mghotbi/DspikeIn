...
#' #   geom_point() +
#' #   scale_color_manual(values = color_palette$extended_palette)
#' @name color_palette
...
color_palette <- local({
  ...
  # Suppress messages when calling external functions like rainbow from grDevices
  extended_palette <- suppressMessages(grDevices::rainbow(50))
  
  # Return as a list
  list(MG = MG, extended_palette = c(MG, extended_palette))
})
