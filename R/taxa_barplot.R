...
#' The top 20 taxa are selected, and the plot can be customized with various options.
...
#' print(bp$barplot)
#' @export
taxa_barplot <- function(physeq, target_glom = "Genus", custom_tax_names = NULL, normalize = TRUE, treatment_variable = "Treatment", abundance_type = "relative", x_angle = 20, fill_variable = target_glom, facet_variable = "Phylum") {
  ...
  # Select top taxa
  top_taxa <- names(sort(taxa_sums(glom_C), decreasing = TRUE)[1:20])
...
# Example usage:
# Generate a taxa barplot for the Genus rank with relative abundance
#bp <- taxa_barplot(Sal, target_glom = "Genus", normalize = TRUE, treatment_variable = "host.genus", abundance_type = "relative", x_angle = 90, fill_variable = "Genus", facet_variable = "Diet")
#print(bp$barplot)

# Generate a taxa barplot for the Genus rank with absolute abundance
#bp <- taxa_barplot(Sal, target_glom = "Genus", normalize = FALSE, treatment_variable = "host.genus", abundance_type = "absolute", x_angle = 90, fill_variable = "Genus", facet_variable = "Diet")
#print(bp$barplot)
