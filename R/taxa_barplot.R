...
#' (OTUs/ASVs) at a specified taxonomic rank. Non-top taxa are aggregated into an "Others" 
#' category, which is always displayed at the top for clarity. The function supports 
#' normalization, faceting, and customization of plot aesthetics. The plot is built using 
#' `ggplot2`, and the taxa are ordered with "Others" appearing first.
#'
...
taxa_barplot <- function(physeq, target_glom = "Genus", custom_tax_names = NULL, 
  ...
  # Step 5: Ensure proper order ("Others" on top, descending for others)
  ...
  # Handle missing taxonomy to avoid "NA" in plots
  new_tax_table <- as.data.frame(phyloseq::tax_table(new_physeq))
  new_tax_table[is.na(new_tax_table)] <- "Unassigned"
  phyloseq::tax_table(new_physeq) <- phyloseq::tax_table(as.matrix(new_tax_table))
  
  # Step 7: Rename taxonomy levels if custom names are provided
  if (!is.null(custom_tax_names)) {
    colnames(phyloseq::tax_table(new_physeq)) <- custom_tax_names
  }
  
  # Step 8: Normalize abundances if requested
  if (normalize && abundance_type == "relative") {
    new_physeq <- phyloseq::transform_sample_counts(new_physeq, function(x) x / sum(x))
  }
  
  # Step 9: Melt data for ggplot
