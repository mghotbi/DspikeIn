...
summ_phyloseq_ASV_OTUID <- function(physeq) {
  suppressMessages({
    ...
    summary_stats <- data.frame(
      ...
      SD = apply(otu_table, 1, stats::sd),          # Standard deviation
      SE = apply(otu_table, 1, function(x) stats::sd(x) / sqrt(length(x))),  # Standard error
      Q25 = apply(otu_table, 1, stats::quantile, probs = 0.25),  # 25th percentile (1st quartile)
      Q50 = apply(otu_table, 1, stats::quantile, probs = 0.5),   # 50th percentile (2nd quartile / median)
      Q75 = apply(otu_table, 1, stats::quantile, probs = 0.75)   # 75th percentile (3rd quartile)
    )
