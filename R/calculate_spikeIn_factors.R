...
#' @importFrom phyloseq sample_data otu_table tax_table prune_taxa merge_taxa sample_sums
...
calculate_spikeIn_factors <- function(physeq, spiked_cells, merged_spiked_species, output_path = NULL) {
  suppressMessages({
    ...
    physeq_no_spiked <- phyloseq::prune_taxa(!phyloseq::tax_table(physeq)[, "Species"] %in% merged_spiked_species, physeq)
    ...
    spiked_16S_total_reads <- data.frame(Sample = rownames(phyloseq::sample_data(physeq)), Total_Reads = phyloseq::sample_sums(physeq))
    utils::write.csv(spiked_16S_total_reads, file.path(output_prefix, "Spiked_16S_Total_Reads.csv"))
    cat("Saved file:", file.path(output_prefix, "Spiked_16S_Total_Reads.csv\n"))
    
    # Subset taxa with specified spiked species
    spiked_species <- phyloseq::prune_taxa(phyloseq::tax_table(physeq)[, "Species"] %in% merged_spiked_species, physeq)
    saveRDS(spiked_species, file.path(output_prefix, "Spiked_Species.rds"))
    
    # Merge spiked species into one
    spiked_species_merged <- phyloseq::merge_taxa(spiked_species, taxa_names(spiked_species))
    ...
    spiked_species_reads <- data.frame(Sample = rownames(phyloseq::sample_data(spiked_species_merged)), Total_Reads = phyloseq::sample_sums(spiked_species_merged))
    utils::write.csv(spiked_species_reads, file.path(output_prefix, "Spiked_Species_Reads.csv"))
    ...
    metadata <- as.data.frame(phyloseq::sample_data(physeq))  # Access sample data
    ...
    utils::write.csv(scaling_factors, file.path(output_prefix, "spikeIn_factors.csv"), row.names = FALSE)
...
# print(physeq_no_spiked)
# print(spiked_16S_total_reads)
